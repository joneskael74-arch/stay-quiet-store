...
export default function App() {
  ...
  return (
    <div style={{ background: '#0b0b0a', color: '#f0ece3', fontFamily: 'Outfit, system-ui, sans-serif', minHeight: '100vh' }}>
      <nav style={{ position: 'fixed', top: 0, left: 0, right: 0, zIndex: 50, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 2.5rem', height: '64px', background: 'rgba(11,11,10,0.88)', backdropFilter: 'blur(12px)', borderBottom: '1px solid #1e1e1b' }}>
        ...
        <button onClick={() => setShowCart(true)} style={{ background: 'none', border: 'none', color: '#f0ece3', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
      ...
      {menuOpen && <div style={{ position: 'fixed', top: '64px', left: 0, right: 0, bottom: 0, zIndex: 40, background: '#0b0b0a', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', gap: '2.5rem' }}>{['Shop', 'Collections', 'About', 'Contact'].map(item => <button key={item} onClick={() => setMenuOpen(false)} style={{ background: 'none', border: 'none', color: '#f0ece3', fontFamily: 'Fraunces, Georgia, serif', fontSize: '2.5rem', fontWeight: 300, letterSpacing: '0.05em', cursor: 'pointer', opacity: 0.85 }}>{item}</button>)}</div>}

      <section style={{ position: 'relative', height: '100vh', overflow: 'hidden', background: '#0b0b0a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at 50% 40%, #1a1a16 0%, #0b0b0a 70%)' }} />
        <div style={{ position: 'relative', zIndex: 1, textAlign: 'center', padding: '0 2rem' }}>
          <img src={logo} alt="" style={{ width: '180px', height: '180px', objectFit: 'cover', mixBlendMode: 'screen', opacity: 0.7, marginBottom: '2rem' }} />
          <h1 style={{ fontFamily: 'Fraunces, Georgia, serif', fontWeight: 300, fontSize: 'clamp(3rem, 9vw, 7rem)', lineHeight: 1, margin: '0 0 1rem', letterSpacing: '-0.02em' }}>Stay <em style={{ fontStyle: 'italic' }}>Quiet</em></h1>
          <p style={{ color: '#6b6760', fontSize: '13px', letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: '2.5rem' }}>Premium Digital Products</p>
          <button onClick={() => document.getElementById('shop')?.scrollIntoView({ behavior: 'smooth' })} style={{ background: 'transparent', border: '1px solid #c9b99a', color: '#c9b99a', padding: '0.85rem 2.5rem', fontSize: '11px', letterSpacing: '0.2em', textTransform: 'uppercase', cursor: 'pointer' }}>Shop Now</button>
        </div>
      </section>

      <section id="shop" style={{ padding: '5rem 6% 6rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '3rem', flexWrap: 'wrap', gap: '1.5rem' }}>
          ...
          <button onClick={() => setShowAdminLogin(true)} style={{ background: 'transparent', border: '1px solid #3a3835', color: '#b8b8b8', padding: '0.5rem 0.8rem', cursor: 'pointer' }}>Admin Login</button>
            <button onClick={() => setShowAddPanel(true)} style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', background: '#141412', border: '1px solid #2e2e2a', color: '#c9b99a', padding: '0.6rem 1.4rem', fontSize: '11px', letterSpacing: '0.15em', textTransform: 'uppercase', cursor: 'pointer' }}>＋ Add Product</button>
        </div>

        {error && <div style={{ marginBottom: '1.5rem', padding: '0.9rem 1rem', border: '1px solid #3a3835', color: '#c9b99a', fontSize: '12px' }}>{error}</div>}
        {loading && <div style={{ textAlign: 'center', padding: '6rem 2rem', color: '#6b6760' }}>Loading products…</div>}
...
const inputStyle: React.CSSProperties = { width: '100%', background: '#0f0f0d', border: '1px solid #252521', color: '#f0ece3', padding: '0.7rem 0.9rem', fontSize: 13, fontFamily: 'Outfit, system-ui, sans-serif', outline: 'none', boxSizing: 'border-box' }
...
const qtyButton: React.CSSProperties = { width: 28, height: 28, background: 'transparent', border: '1px solid #2e2e2a', color: '#f0ece3', cursor: 'pointer' }
